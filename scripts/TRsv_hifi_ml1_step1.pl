        ...
        foreach my $ipos (keys %ins){
            ...
            if ($ilen >= $max_dist){
                ...
                if (exists ${$bam_ins{$Mbin}}{$ipos}){
                    ...
                    my $ave = int ($sumlen / @{${$bam_ins{$Mbin}}{$ipos}});
                    if (($ave / $ilen <= 1.7) and ($ave / $ilen >= 0.59)){
                        push @{${$bam_ins{$Mbin}}{$ipos}}, "$ins{$ipos}=$ipos";
                    }
                    else{
                        my $ipos2 = $ipos;
                        while (1){
                            ...
                            if (exists ${$bam_ins{$Mbin2}}{$ipos2}){
                                ...
                                if (($ilen2 / $ilen <= 1.7) and ($ilen2 / $ilen >= 0.59)){
                                    push @{${$bam_ins{$Mbin2}}{$ipos2}}, "$ins{$ipos}=$ipos2";
                                    last;
                                }
                            }
                            else{
            ...
            if ($dlen >= $max_dist){
                if (exists ${$bam_del{$Mbin}}{$dpos}){
                    ...
                    if (($ave / $dlen <= 1.7) and ($ave / $dlen >= 0.59)){
                        push @{${$bam_del{$Mbin}}{$dpos}}, "$dlen=$dpos=$read_id";
                    }
                    else{
                        my $dpos2 = $dpos;
                        ...
                        while (1){
                            ...
                            if (exists ${$bam_del{$Mbin2}}{$dpos2}){
                                ...
                                if (($dlen3 / $dlen <= 1.7) and ($dlen3 / $dlen >= 0.59)){
                                    push @{${$bam_del{$Mbin2}}{$dpos2}}, "$dlen=$dpos2=$read_id";
                                    last;
                                }
                            }
                            else{
...
foreach my $sid (keys %bam_str){        # assign the sizes and counts (genotype) of INSs and DELs within TRs
    ...
    if (($del_rate > 0) and ($ins_rate + $bp_rate > 0) and ($del_rate + $ins_rate + $bp_rate >= $min_str_vrr * 0.3)){
        ...
        if ($sum_ins == 1){
            ...
            if (($slen >= 200) and ($term_bp5 >= $sum_read_all * 0.15) and ($term_bp3 >= $sum_read_all * 0.15)){     # assign long INS overlapping STR DUP
                my ($flank_dp) = &calc_dp ($spos, $send, \%DP, 'STR');
                $dprate = int ($sum_read_all / $flank_dp * 100 + 0.5) / 100 if ($flank_dp > 0);
                if ($dprate >= 1.2){
                    my $duplen = 0;
...
if ($min_indel_size <= 3){
    ...
    foreach my $bin (sort {$a <=> $b} keys %bam_ins){
        foreach my $pos (sort {$a <=> $b} keys %{$bam_ins{$bin}}){
            ...
            if (($med_len == 3) and ($min_indel_size == 1)){
                @len = ();
                my @new_info;
                my $remove = 0;
                foreach (@{${$bam_ins{$bin}}{$pos}}){
            ...
            if (($med_len <= 3) and ((@len < 3) or (@len < $min_ins_reads))){
                delete ${$bam_ins{$bin}}{$pos};
                next;
            }
            my $read_num = scalar @len;
            my ($dp) = &calc_dp ($pos, $pos, \%DP, 'INS');
            $dp = 10 if ($dp == 0);
            my $vrr = int ($read_num / $dp * 100 + 0.5) / 100;
            if ($med_len <= 2){
                if ($vrr < 0.2){
                    delete ${$bam_ins{$bin}}{$pos};
                    next;
                }
                elsif (($read_num < 5) and ($vrr < 0.3)){
                    delete ${$bam_ins{$bin}}{$pos};
                    next;
                }
            }
    ...
    foreach my $pos (sort {$a <=> $b} keys %{$bam_ins{$bin}}){ 
        ...
        if ($diff >= 1.3){
            ...
            foreach my $pos2 (sort {$a <=> $b} keys %{$bam_ins{$start_bin}}){
                ...
                next if ($pos2 == $pos);
                my $bin2 = int ($pos2 / $Mbin_size);
                next if (!exists ${$bam_ins{$bin2}}{$pos2});
                foreach (@{${$bam_ins{$bin2}}{$pos2}}){
                    my ($id, $strand, $ilen) = split (/=/, $_);
                    ${$pos2_ids{$id}}{$pos2} = $ilen;
                }
            }
            if (($end_bin > $start_bin) and (exists $bam_ins{$end_bin})){
            ...
            if (scalar keys %pos2_ids > 0){
                ...
                if (scalar keys %remove_ins > 0){
                    ...
                    foreach my $pos2 (keys %remove_ins){
                        my $bin2 = int ($pos2 / $Mbin_size);
                        if (@{$remove_ins{$pos2}} == @{${$bam_ins{$bin2}}{$pos2}}){
                            delete ${$bam_ins{$bin2}}{$pos2};
                        }
                        else{
                            my %ids;
                            my @info2;
    ...
    foreach my $pos1 (sort {$a <=> $b} keys %{$bam_ins{$bin}}){ 
        ...
        if (scalar keys %hit > 0){
            ...
            if (exists $delete{$med_pos}){
                while (1){
                    $med_pos ++;
                    my $Mbin = int ($med_pos / $Mbin_size);
                    last if (!exists $delete{$med_pos}) and (!exists ${$bam_ins{$Mbin}}{$med_pos});
                }
            }
            $hit{$pos1} = $num1;
            my @len;
...
if ($min_indel_size <= 3){
    ...
    foreach my $bin (sort {$a <=> $b} keys %bam_del){
        foreach my $pos (sort {$a <=> $b} keys %{$bam_del{$bin}}){
            next if (!exists ${$bam_del{$bin}}{$pos});
            my @len;
            ...
            next if ($medlen > 3);
            my $end = $pos + $medlen - 1;
            if (($pre_pos > 0) and ($pre_end - $pos >= -1)){
                if (@{${$bam_del{$pre_bin}}{$pre_pos}} >= @{${$bam_del{$bin}}{$pos}}){
                    push @{${$bam_del{$pre_bin}}{$pre_pos}}, @{${$bam_del{$bin}}{$pos}};
                    delete ${$bam_del{$bin}}{$pos};
                    next;
                }
                else{
                    push @{${$bam_del{$bin}}{$pos}}, @{${$bam_del{$pre_bin}}{$pre_pos}};
                    delete ${$bam_del{$pre_bin}}{$pre_pos};
                }
            }
            $pre_pos = $pos;
            $pre_end = $end;
    ...
    foreach my $bin (sort {$a <=> $b} keys %bam_del){
        foreach my $pos (sort {$a <=> $b} keys %{$bam_del{$bin}}){
            ...
            @len = sort {$a <=> $b} @len;
            my $medlen = $len[$hn];
            if ($medlen < $min_indel_size){
                delete ${$bam_del{$bin}}{$pos};
                next;
            }
            if (($medlen == 3) and ($min_indel_size == 1)){
                @len = ();
                my @new_info;
                my $remove = 0;
                foreach (@{${$bam_del{$bin}}{$pos}}){
                ...
                if ($remove > 0){
                    @{${$bam_del{$bin}}{$pos}} = (@new_info);
                }
            }
            if (($medlen <= 3) and ((@len < 3) or (@len < $min_del_reads))){
                delete ${$bam_del{$bin}}{$pos};
                next;
            }
            my $read_num = scalar @len;
            my $end = $pos + $medlen;
            my ($dp) = &calc_dp ($pos, $end, \%DP, 'DEL');
            $dp = 10 if ($dp == 0);
            my $vrr = int ($read_num / $dp * 100 + 0.5) / 100;
            if ($medlen <= 2){
                if ($vrr < 0.2){
...
foreach my $bin (sort {$a <=> $b} keys %bam_del){
    foreach my $pos1 (sort {$a <=> $b} keys %{$bam_del{$bin}}){
        ...
        %merge = ();
        if (($bin2 > $bin) and (exists $bam_del{$bin2})){
...
foreach my $bin (sort {$a <=> $b} keys %bam_del){       # devide or delete highly deviated size of INSs
    foreach my $pos (sort {$a <=> $b} keys %{$bam_del{$bin}}){ 
        ...
        next if ($med_len <= 3);
        my $diff = int ($max_len / $min_len * 100 + 0.5) / 100;
    #print STDERR "0: $pos diff: $diff\t@len\n" if ($pos >= 6650000) and ($pos <= 6653800);
        if ($diff <= 1.5){
            my $hn = int (@len * 0.5);
            @len = sort {$a <=> $b} @len;
            my $med_len = $len[$hn];
...
foreach my $bin (sort {$a <=> $b} keys %bam_del){   # merge clustered dels for each read
    foreach my $pos (sort {$a <=> $b} keys %{$bam_del{$bin}}){ 
        ...
        foreach my $pos2 (sort {$a <=> $b} keys %{$bam_del{$bin}}){
            ...
            my $num2 = scalar @{${$bam_del{$bin}}{$pos2}};
            ...
            if ($pos2 < $end){
                ...
                if (($overlap >= $len * $min_overlap_rate2) and ($overlap >= $len2 * $min_overlap_rate2)){
                    if (!exists $clust_del{$pos}){
                        push @{$clust_del{$pos}}, @{${$bam_del{$bin}}{$pos}} if (exists ${$bam_del{$bin}}{$pos});
                        push @{$clust_del{$pos}}, @{${$bam_del{$bin}}{$pos2}};
                        $used_del{$pos} = 1;
                        $used_del{$pos2} = 1;
                    }
                    else{
        ...
        if (($bin2 > $bin) and (exists $bam_del{$bin2})){
            foreach my $pos2 (sort {$a <=> $b} keys %{$bam_del{$bin2}}){
                ...
                my $num2 = scalar @{${$bam_del{$bin2}}{$pos2}};
                my $len2 = int ($sumlen2 / $num2 + 0.5);
                my $end2 = $pos2 + $len2 - 1;
                if ($pos2 < $end){
                    my $overlap = $end - $pos2 + 1;
                    ...
                    if (($overlap >= $len * $min_overlap_rate2) and ($overlap >= $len2 * $min_overlap_rate2)){
                        if (!exists $clust_del{$pos}){
                            push @{$clust_del{$pos}}, @{${$bam_del{$bin2}}{$pos}} if (exists ${$bam_del{$bin2}}{$pos});
                            push @{$clust_del{$pos}}, @{${$bam_del{$bin2}}{$pos2}};
                            $used_del{$pos} = 1;
                            $used_del{$pos2} = 1;
                        }
                        else{
...
foreach my $bin (keys %bam_del){
    foreach my $pos (keys %{$bam_del{$bin}}){
        delete ${$bam_del{$bin}}{$pos} if (exists $used_del{$pos});
    }
}
...
foreach my $pos (sort {$a <=> $b} keys %clust_del){
    ...
    my $bin1 = int ($ave_pos / $Mbin_size);
    if (!exists ${$bam_del{$bin1}}{$ave_pos}){
        push @{${$bam_del{$bin1}}{$ave_pos}}, @{$clust_del{$pos}};
    }
    else{
        while (1){
            $ave_pos ++;
            my $bin2 = int ($ave_pos / $Mbin_size);
            next if (exists ${$bam_del{$bin2}}{$ave_pos});
            push @{${$bam_del{$bin2}}{$ave_pos}}, @{$clust_del{$pos}};
            last;
        }
    }
}
            ...
            if ($pos2 < $end){
                ...
                if (($overlap >= $len * $min_overlap_rate) and ($overlap >= $len2 * $min_overlap_rate)){
                    if (!exists $clust_del2{$pos}){
                        push @{$clust_del2{$pos}}, @{${$bam_del{$bin}}{$pos}} if (exists ${$bam_del{$bin}}{$pos});
                        push @{$clust_del2{$pos}}, @{${$bam_del{$bin}}{$pos2}};
                        $used_del2{$pos} = 1;
                        $used_del2{$pos2} = 1;
                    }
                    else{
        ...
        if (($bin2 > $bin) and (exists $bam_del{$bin2})){
            foreach my $pos2 (sort {$a <=> $b} keys %{$bam_del{$bin2}}){
                ...
                if ($pos2 < $end){
                    ...
                    if (($overlap >= $len * $min_overlap_rate) and ($overlap >= $len2 * $min_overlap_rate)){
                        if (!exists $clust_del2{$pos}){
                            push @{$clust_del2{$pos}}, @{${$bam_del{$bin2}}{$pos}} if (exists ${$bam_del{$bin2}}{$pos});
                            push @{$clust_del2{$pos}}, @{${$bam_del{$bin2}}{$pos2}};
                            $used_del2{$pos} = 1;
                            $used_del2{$pos2} = 1;
                        }
                        else{
...
foreach my $bin (keys %bam_del){
    foreach my $pos (keys %{$bam_del{$bin}}){
        delete ${$bam_del{$bin}}{$pos} if (exists $used_del2{$pos});
    }
}
...
foreach my $pos (sort {$a <=> $b} keys %clust_del2){
    ...
    my $bin1 = int ($ave_pos / $Mbin_size);
    if (!exists ${$bam_del{$bin1}}{$ave_pos}){
        push @{${$bam_del{$bin1}}{$ave_pos}}, @{$clust_del2{$pos}};
    }
    else{
        while (1){
            $ave_pos ++;
            my $bin2 = int ($ave_pos / $Mbin_size);
            next if (exists ${$bam_del{$bin2}}{$ave_pos});
            push @{${$bam_del{$bin2}}{$ave_pos}}, @{$clust_del2{$pos}};
            last;
        }
    }
}
...
foreach my $bin (sort {$a <=> $b} keys %bam_del){       # devide or delete highly deviated size of DELs
    foreach my $pos (sort {$a <=> $b} keys %{$bam_del{$bin}}){ 
        ...
        if ($avelen < $min_indel_size){
            ...
            my $bin2 = int ($max_med_pos / $Mbin_size);
            my $bin3 = int ($min_med_pos / $Mbin_size);
            delete ${$bam_del{$bin}}{$pos};
            push @{${$bam_del{$bin2}}{$max_med_pos}}, @max;
            push @{${$bam_del{$bin3}}{$min_med_pos}}, @min;
        }
    }
}
...
foreach my $bp1 (sort {$a <=> $b} keys %bam_del_bp){        # remove BP-DELs overlapping assigned DELs
    ...
    if (($bin3 > $bin2) and (exists $bam_del{$bin3})){
...
foreach my $pos (sort {$a <=> $b} keys %ins_bp12){      # assign INS to INS-BP with BPLEN or INS-BP to DUP
    ...
    if ($bplen > 100){
        ...
        if (($Mbin2 > $Mbin1) and (exists $bam_ins{$Mbin2})){
    ...
    if ($max_len == 0){
        ...
        if ($max_len / $min_len >= 1.5){
            ...
            foreach my $rid (keys %select_read){
                ...
                my $ipos = $read_ipos{$rid};
                my $Mbin = int ($ipos / $Mbin_size);
                push @{$ipos{$ipos}}, @{${$bam_ins{$Mbin}}{$ipos}} if (exists ${$bam_ins{$Mbin}}{$ipos});
            }
        }
        my @info;
...
foreach my $bin (sort {$a <=> $b} keys %bam_del){
    foreach my $pos (sort {$a <=> $b} keys %{$bam_del{$bin}}){
        ...
        my $flank_dp = 0;
        if ($ave_len >= 50){
            ($dp_rate, $flank_dp) = &calc_dprate ($pos, $ave_end, \%DP, 'DEL');
        }
        else{
            if (exists $shortDEL_DP{$pos}){
                $flank_dp = $shortDEL_DP{$pos};
            }
            else{
