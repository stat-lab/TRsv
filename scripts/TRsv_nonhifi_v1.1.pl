                    ...
                    if ($type eq 'DEL'){
                        ...
                        foreach my $rpos (keys %hit_rpos){
                            ...
                            if (exists ${${$sv2{$chr}}{$rpos}}{'TR'}){
                                ...
                                my $str_line = "$chr2\t$rpos\t.\t.\t<TR:CNV>\t.\tPASS\tSVTYPE=INS;SVLEN=$ins_len;READS=$read;ßßgain+$cn;VRR=$vrr;SAR=$sar;GT=$gt;END=$rend;TRID=$strid;TREND=$rend;TRULEN=$mlen;OVLSV=$pos-$type-$len";
                                ${${$sv2{$chr}}{$rpos}}{'TR'} = $str_line;
                                $added_str_ins ++;
                            }
                        }
...
print OUT "##INFO=<ID=OVLSV,Number=1,Type=String,Description=\"Position-Type-Size of DUP or DEL overlapping the TR region to be assigned as TR-INS or TR-DEL\">\n";
...
if ($filt_flag == 1){
    ...
    print OUTF "##INFO=<ID=OVLSV,Number=1,Type=String,Description=\"Position-Type-Size of DUP or DEL overlapping the TR region to be assigned as TR-INS or TR-DEL\">\n";
    print OUTF "##INFO=<ID=GT,Number=1,Type=String,Description=\"Genotype of SV (HT/HT2/HM/NA, HT2: multiallelic genotype)\">\n";
