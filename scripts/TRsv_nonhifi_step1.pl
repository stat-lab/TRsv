...
if ($platform eq 'pacbio'){
    $min_del_size = 30;
    $min_ins_size = 45;
    $min_del_size = $min_indel_size * 0.9 if ($min_del_size > $min_indel_size);
    $min_ins_size = $min_indel_size * 0.9 if ($min_ins_size > $min_indel_size);
    $min_del_size = $min_str_indel_size * 0.8 if ($min_del_size > $min_str_indel_size);
    $min_ins_size = $min_str_indel_size * 0.8 if ($min_ins_size > $min_str_indel_size);
}
...
while (my $line = <FILE>){
    ...
    $sum_align += $map_len;
    if ($cigar =~ /^(\d+)[SH].+[MIDX=](\d+)[SH]$/){
        my $term5len = $1;
        my $term3len = $2;
        my $alignlen = 0;
        if (($term5len >= $min_clip_len2) and ($term3len >= $min_clip_len2)){
            while ($cigar =~ /(\d+)[MIX=]/g){
                $alignlen += $1;
            }
            my $readlen = $alignlen + $term5len + $term3len;
            my $align_rate = int ($alignlen / $readlen * 100 + 0.5) / 100;
            if ($align_rate < $min_align_rate){
                $delete_alignrate ++;
                next;
            }
        }
    ...
    while ($cigar =~ /(\d+)([SHMIDX=])/g){
        ...
        if (($tag =~ /S|H/) and ($count == 1)){
            if ($sublen >= $min_clip_len){
                $S5_flag = $sublen;
                $sublen_5S = $sublen;
                push @{$bam_bp2{$pos}}, "$read_id=$strand=$sublen=$tag=$map_len=$pos" if (!exists ${$gap_bp{$chr}}{$pos});
                ${$read_bp2{$read_id}}{$pos} = "$map_len=$sublen=$cigar=$strand" if (!exists ${$gap_bp{$chr}}{$pos});
                ${$shortmap{$read_id}}{$pos} = 1 if ($map_len < $min_maplen * 2);
            }
            $read_pos += $sublen;
            next;
        }
        if (($tag eq 'M') or ($tag eq '=')){
            ...
            if ($sublen >= $min_ins_size){
                $ins{$end} = "$read_id=$strand=$sublen=$read_pos";
            }
            $read_pos += $sublen;
        }
        elsif ($tag eq 'X'){
            if ($sublen >= $min_indel_size){
                my $prefix = $`;
                my $suffix = $';
                if (($prefix =~ /[M=]$/) and ($suffix =~ /^\d+[M=]/)){
                    push @{$bam_rep{$end}}, $sublen;
                }
            }
            $end += $sublen;
            $read_pos += $sublen;
            ...
            if (($map_len >= $min_maplen) and (!exists ${$gap_bp{$chr}}{$end})){
                push @{$bam_bp1{$end}}, "$read_id=$strand=$sublen=$tag=$map_len=$end";
                ${$read_bp1{$read_id}}{$end} = "$read_pos=$sublen=$cigar=$strand";
                $read_bp1_num1 ++;
            }
            ${$shortmap{$read_id}}{$end} = 1 if ($map_len < $min_maplen * 2);
    ...
    my %str_bp_flag;
    my %str_bp_flag2;
    foreach my $sbin (sort {$a <=> $b} keys %repeat){      # determine read coverage and the number of breal ends within TRs
        last if ($sbin > $Mbin_e);
        next if ($sbin < $Mbin_s);
    ...
    if (scalar keys %ins > 0){
        foreach my $ipos (sort {$a <=> $b} keys %ins){      # assign INSs (>= 20 bp) within TRs
            ...
            if (exists $repeat{$Mbin_start}){
                ...
                foreach my $spos (sort {$a <=> $b} keys %{$repeat{$Mbin_start}}){
                    ...
                    my $slen = $send - $spos + 1;
                    last if ($spos > $ipos + $diff);
                    next if ($send < $ipos - $diff);
                    my $str = $STR{$spos};
                    $STRins{$ipos} = $str;
                    ${$STRins_id{$str}}{$ipos} = $ilen;
                    $match_str{$str} = 1;
                    $STR_id{$str} = 1;
                }
        ...
        foreach my $ipos (sort {$a <=> $b} keys %ins){
            ...
            elsif (($distance <= 50) and ($distance < $ilen * 0.5) and ($distance < $pre_len * 0.5)){
                $flag = 1;
            }
            elsif (($distance <= 100) and ($distance < $ilen * 0.2) and ($distance < $pre_len * 0.2)){
                $flag = 1;
            }
        ...
        foreach my $ipos (keys %ins){
            next if (exists $STRins{$ipos});
            my ($id, $istrand, $ilen) = split (/=/, $ins{$ipos});
            if ($ilen >= $min_indel_size * 0.8){
                my $Mbin = int ($ipos / $Mbin_size);
                if (exists ${$bam_ins{$Mbin}}{$ipos}){
                    my $sumlen = 0;
                    foreach (@{${$bam_ins{$Mbin}}{$ipos}}){
                        my ($id1, $strand1, $ilen1) = split (/=/, $_);
                        $sumlen += $ilen1;
                    }
                    my $num = scalar @{${$bam_ins{$Mbin}}{$ipos}};
                    my $ave = int ($sumlen / $num);
                    if (($ave / $ilen <= 1.7) and ($ave / $ilen >= 0.59)){
                        push @{${$bam_ins{$Mbin}}{$ipos}}, "$ins{$ipos}=$ipos=$mapQ";
                    }
                    else{
                        my $ipos2 = $ipos;
                        while (1){
                            $ipos2 ++;
                            my $Mbin2 = int ($ipos2 / $Mbin_size);
                            if (exists ${$bam_ins{$Mbin2}}{$ipos2}){
                                my ($id2, $strand2, $ilen2) = split (/=/, ${${$bam_ins{$Mbin2}}{$ipos2}}[0]);
                                if (($ilen2 / $ilen <= 1.7) and ($ilen2 / $ilen >= 0.59)){
                                    push @{${$bam_ins{$Mbin2}}{$ipos2}}, "$ins{$ipos}=$ipos2=$mapQ";
                                    last;
                                }
                            }
                            else{
    ...
    if (scalar keys %del > 0){
        ...
        foreach my $dpos1 (sort {$a <=> $b} keys %del){
            ...
            while (1){
                ...
                foreach my $dpos2 (sort {$a <=> $b} keys %del){
                    ...
                    elsif ((($dlen2 / $dlen1 >= 10) or ($dlen1 / $dlen2 >= 10)) and ($distance < $dlen1 * 0.2) and ($distance < $dlen2 * 0.2) and ($distance < 200)){
                        $flag = $dpos2;
                    }
                    push @dpos2, $dpos2;
                }
        ...
        foreach my $dpos (keys %del){
            ...
            my $dlen = $del{$dpos};
            my $Mbin = int ($dpos / $Mbin_size);
            if ($dlen >= $min_indel_size * 0.8){
                if (exists ${$bam_del{$Mbin}}{$dpos}){
                    my $sumlen = 0;
                    foreach (@{${$bam_del{$Mbin}}{$dpos}}){
                        my ($dlen2) = split (/=/, $_);
                        $sumlen += $dlen2;
                    }
                    my $ave = int ($sumlen / @{${$bam_del{$Mbin}}{$dpos}});
                    if (($ave / $dlen <= 1.8) and ($ave / $dlen >= 0.56)){
                        push @{${$bam_del{$Mbin}}{$dpos}}, "$dlen=$dpos=$read_id=$mapQ";
                    }
                    else{
                        my $dpos2 = $dpos;
                        while (1){
                            $dpos2 ++;
                            my $Mbin2 = int ($dpos2 / $Mbin_size);
                            if (exists ${$bam_del{$Mbin2}}{$dpos2}){
                                my ($dlen3) = split (/=/, ${${$bam_del{$Mbin2}}{$dpos2}}[0]);
                                if (($dlen3 / $dlen <= 1.8) and ($dlen3 / $dlen >= 0.56)){
                                    push @{${$bam_del{$Mbin2}}{$dpos2}}, "$dlen=$dpos2=$read_id=$mapQ";
                                    last;
                                }
                            }
                            else{
    ...
    foreach my $sid (keys %STR_id){                         # Take either INSs or DELs, that were mapped in a read aligned within a TR, and estimate a merged size of fragmented INSs mapped within a read within a TR
...
if ($targeted_seq == 0){
    ...
    if (scalar keys %high_depth_range > 0){
        foreach my $dpos (sort {$a <=> $b} keys %high_depth_range){
            ...
            foreach my $bin (sort {$a <=> $b} keys %bam_ins){
                foreach my $pos (sort {$a <=> $b} keys %{$bam_ins{$bin}}){
                    last if ($pos > $dend);
                    next if ($pos < $dpos);
                    delete ${$bam_ins{$bin}}{$pos};
                }
            }
            foreach my $bin (sort {$a <=> $b} keys %bam_del){
                foreach my $pos (sort {$a <=> $b} keys %{$bam_del{$bin}}){
                    last if ($pos > $dend - 50);
                    next if ($pos < $dpos);
                    delete ${$bam_del{$bin}}{$pos};
                }
            }
...
foreach my $sid (keys %bam_str){        # assign the sizes and counts (genotype) of INSs and DELs within TRs
    ...
    if (($del_rate > 0) and ($ins_rate + $bp_rate > 0) and ($del_rate + $ins_rate + $bp_rate >= $min_str_vrr * 0.3)){
        ...
        my ($med_lenD1, $med_lenD2, $var_numD1, $var_numD2) = &clust_indel (\@del_len);
        my ($med_lenI1, $med_lenI2, $var_numI1, $var_numI2) = (0, 0, 0, 0);
        if ($ins_rate > 0){
            my @ins_len = @{${$bam_str{$sid}}{'INS'}};
            @ins_len = @{${$bam_str2{$sid}}{'INS'}} if ($flag2 == 1);
            ($med_lenI1, $med_lenI2, $var_numI1, $var_numI2) = &clust_indel (\@ins_len);
        }
        ...
        my @del_len = @{${$bam_str{$sid}}{'DEL'}};
        @del_len = @{${$bam_str2{$sid}}{'DEL'}} if ($flag2 == 1);
        ($med_len1, $med_len2, $var_num1, $var_num2) = &clust_indel (\@del_len);
        $type1 = 'DEL';
        ...
        if ($sum_ins == 1){
            ...
            my @ins_len = @{${$bam_str{$sid}}{'INS'}};
            @ins_len = @{${$bam_str2{$sid}}{'INS'}} if ($flag2 == 1);
            ($med_len1, $med_len2, $var_num1, $var_num2) = &clust_indel (\@ins_len);
            ...
            if (($slen >= 200) and ($term_bp5 >= $sum_read_all * 0.2) and ($term_bp3 >= $sum_read_all * 0.2)){
                my ($flank_dp) = &calc_dp ($spos, $send, \%DP, 'STR');
                $dprate = int ($sum_read_all / $flank_dp * 100 + 0.5) / 100 if ($flank_dp > 0);
                if ($dprate < 1.2){
                    if (exists ${$bam_str{$sid}}{'BP5'}){
    ...
    if ($type1 eq 'INS'){
        foreach my $readid (keys %{$bam_str_insinfo{$sid}}){
            ...
            if (($ilen / $med_len1 <= 1.2) and ($ilen / $med_len1 >= 0.8)){
                push @{${$bam_ins_str_read{$spos}}{1}}, "$readid~$ipos_str~$ipos";
            }
        }
        if ((!exists ${$bam_ins_str_read{$spos}}{1}) or (@{${$bam_ins_str_read{$spos}}{1}} == 0)){
            foreach my $readid (keys %{$bam_str_insinfo{$sid}}){
                my ($ilen, $ipos_str, $ipos) = split (/=/, ${$bam_str_insinfo{$sid}}{$readid});
    ...
    if (($type2 eq 'INS') and ($med_len2 > 0)){
        foreach my $readid (keys %{$bam_str_insinfo{$sid}}){
            ...
            if (($ilen / $med_len2 <= 1.2) and ($ilen / $med_len2 >= 0.8)){
                push @{${$bam_ins_str_read{$spos}}{2}}, "$readid~$ipos_str~$ipos";
            }
        }
        if ((!exists ${$bam_ins_str_read{$spos}}{2}) or (@{${$bam_ins_str_read{$spos}}{2}} == 0)){
            foreach my $readid (keys %{$bam_str_insinfo{$sid}}){
                my ($ilen, $ipos_str, $ipos) = split (/=/, ${$bam_str_insinfo{$sid}}{$readid});
...
foreach my $bin (sort {$a <=> $b} keys %bam_ins){
    foreach my $pos (sort {$a <=> $b} keys %{$bam_ins{$bin}}){
        if (($pre_pos > 0) and ($pos - $pre_pos <= $ins_bp_diff2)){
            if (@{${$bam_ins{$pre_bin}}{$pre_pos}} >= @{${$bam_ins{$bin}}{$pos}}){
                push @{${$bam_ins{$pre_bin}}{$pre_pos}}, @{${$bam_ins{$bin}}{$pos}};
                delete ${$bam_ins{$bin}}{$pos};
                next;
            }
            else{
                push @{${$bam_ins{$bin}}{$pos}}, @{${$bam_ins{$pre_bin}}{$pre_pos}};
                delete ${$bam_ins{$pre_bin}}{$pre_pos};
            }
        }
        $pre_pos = $pos;
        $pre_bin = $bin;
...
foreach my $bin (sort {$a <=> $b} keys %bam_ins){       # devide or delete highly deviated size of INSs
    foreach my $pos (sort {$a <=> $b} keys %{$bam_ins{$bin}}){
        next if (!exists ${$bam_ins{$bin}}{$pos});
        my $max_len = 0;
        my $min_len = 10000000000;
        my @len;
        foreach (@{${$bam_ins{$bin}}{$pos}}){
        ...
        if ($diff >= 1.3){
            ...
            my $start = $pos - $mergin;
            my $end = $pos + $mergin;
            my $start_bin = int ($start / $Mbin_size);
            my $end_bin = int ($end / $Mbin_size);
            foreach my $pos2 (sort {$a <=> $b} keys %{$bam_ins{$start_bin}}){
                last if ($pos2 > $pos + $mergin);
                next if ($pos2 < $pos - $mergin);
                next if ($pos2 == $pos);
                my $Mbin = int ($pos2 / $Mbin_size);
                next if (!exists ${$bam_ins{$Mbin}}{$pos2});
                foreach (@{${$bam_ins{$Mbin}}{$pos2}}){
                    my ($id, $strand, $ilen) = split (/=/, $_);
                    ${$pos2_ids{$id}}{$pos2} = $ilen;
                }
            }
            if ($end_bin > $start_bin){
                foreach my $pos2 (sort {$a <=> $b} keys %{$bam_ins{$end_bin}}){
                    last if ($pos2 > $pos + $mergin);
                    next if ($pos2 < $pos - $mergin);
                    next if ($pos2 == $pos);
                    my $Mbin = int ($pos2 / $Mbin_size);
            ...
            if (scalar keys %pos2_ids > 0){
                ...
                my @new_info;
                foreach (@{${$bam_ins{$bin}}{$pos}}){
                    my ($id, $strand, $ilen, $rpos, $ipos, $mapq) = split (/=/, $_);
                    if (exists $pos2_ids{$id}){
                        ...
                        if (($ilen3 / $max_len >= 0.9) and ($ilen3 / $max_len <= 1.11)){
                            ...
                            push @new_info, "$id=$strand=$ilen3=$rpos=$ipos=$mapq";
                        }
                        else{
                            push @new_info, $_;
                        }
                ...
                if (scalar keys %remove_ins > 0){
                    @{${$bam_ins{$bin}}{$pos}} = (@new_info);
                    foreach my $pos2 (keys %remove_ins){
                        my $Mbin = int ($pos2 / $Mbin_size);
                        if (@{$remove_ins{$pos2}} == @{${$bam_ins{$Mbin}}{$pos2}}){
                            delete ${$bam_ins{$Mbin}}{$pos2};
                        }
                        else{
                            my %ids;
                            my @info2;
        ...
        if ($med_minlen < $min_indel_size){
            ...
            my $min_bin = int ($med_minpos / $Mbin_size);
            my $max_bin = int ($med_maxpos / $Mbin_size);
            if ($med_minlen < $min_indel_size){
                push @{${$bam_ins{$max_bin}}{$med_maxpos}}, @max;
                $bam_ins_len{$med_maxpos} = $med_maxlen;
                next;
            }
            push @{${$bam_ins{$min_bin}}{$med_minpos}}, @min;
            push @{${$bam_ins{$max_bin}}{$med_maxpos}}, @max;
...
foreach my $bin (sort {$a <=> $b} keys %bam_ins){          # merge similar size of INS with distance 10-150 bp
    foreach my $pos1 (sort {$a <=> $b} keys %{$bam_ins{$bin}}){
        next if (exists $delete{$pos1});
        next if (!exists ${$bam_ins{$bin}}{$pos1});
        my $num1 = scalar @{${$bam_ins{$bin}}{$pos1}};
        my $len1 = $bam_ins_len{$pos1};
        my @len;
        my %ids;
        foreach (@{${$bam_ins{$bin}}{$pos1}}){
            my ($id, $strand, $ilen) = split (/=/, $_);
            push @len, $ilen;
            $ids{$id} = 1;
        }
        if (@len == 0){
            delete ${$bam_ins{$bin}}{$pos1};
            next;
        }
        @len = sort {$a <=> $b} @len;
        my $hn = int (@len * 0.5);
        ...
        $bam_ins_len{$pos1} = $len1 if (!exists $bam_ins_len{$pos1});
        my $max_distance = $len1 * 0.2;
        $max_distance = 150 if ($max_distance > 150);
        my $end1 = $pos1 + $max_distance;
        my $end_bin = int ($end1 / $Mbin_size);
        my %hit;
        foreach my $pos2 (sort {$a <=> $b} keys %{$bam_ins{$bin}}){
            next if (exists $delete{$pos2});
            next if ($pos2 <= $pos1);
            last if ($pos2 > $end1);
            my $num2 = scalar @{${$bam_ins{$bin}}{$pos2}};
            my $len2 = $bam_ins_len{$pos2};
            my @len;
            my $share_id = 0;
            foreach (@{${$bam_ins{$bin}}{$pos2}}){
                my ($id, $strand, $ilen) = split (/=/, $_);
                push @len, $ilen;
                $share_id ++ if (exists $ids{$id});
            }
            if (@len == 0){
                delete ${$bam_ins{$bin}}{$pos2};
                next;
            }
            @len = sort {$a <=> $b} @len;
            my $hn = int (@len * 0.5);
            ...
            $bam_ins_len{$pos2} = $len2 if (!exists $bam_ins_len{$pos2});
            my $rate = int ($len1 / $len2 * 100 + 0.5) / 100;
            if (($rate >= 0.8) and ($rate <= 1.2) and ($share_id < $num1 * 0.5) and ($share_id < $num2 * 0.5)){
                $hit{$pos2} = $num2;
            }
        }
        if (($end_bin - 1 == $bin) and (scalar keys %hit == 0)){
            foreach my $pos2 (sort {$a <=> $b} keys %{$bam_ins{$end_bin}}){
                next if (exists $delete{$pos2});
                next if ($pos2 <= $pos1);
                last if ($pos2 > $end1);
                my $num2 = scalar @{${$bam_ins{$end_bin}}{$pos2}};
                my $len2 = $bam_ins_len{$pos2};
                my @len;
                my $share_id = 0;
                foreach (@{${$bam_ins{$bin}}{$pos2}}){
        ...
        if (scalar keys %hit > 0){
            ...
            if (exists $delete{$med_pos}){
                while (1){
                    $med_pos ++;
                    last if (!exists $delete{$med_pos}) and (!exists ${$bam_ins{$bin}}{$med_pos}) and (!exists ${$bam_ins{$end_bin}}{$med_pos});
                }
            }
            $hit{$pos1} = $num1;
            my $num = 0;
            my $sum_len = 0;
            my $med_bin = int ($med_pos / $Mbin_size);
            if (exists ${$bam_ins{$med_bin}}{$med_pos}){
                $num = scalar @{${$bam_ins{$med_bin}}{$med_pos}};
                $sum_len = $bam_ins_len{$med_pos};
            }
            foreach my $pos (keys %hit){
                if ($pos != $med_pos){
                    my $bin2 = int ($pos / $Mbin_size);
                    push @{${$bam_ins{$med_bin}}{$med_pos}}, @{${$bam_ins{$bin2}}{$pos}} if (!exists $delete{$pos});
                    $sum_len += $bam_ins_len{$pos};
                    $num += scalar @{${$bam_ins{$bin2}}{$pos}};
                    delete ${$bam_ins{$bin2}}{$pos};
                    $delete{$pos} = 1;
                }
            }
            my $ave_len = int ($sum_len / $num + 0.5);
            if ($ave_len == 0){
                delete ${$bam_ins{$med_bin}}{$med_pos};
                next;
            }
            $bam_ins_len{$med_pos} = $ave_len;
        } 
    }
}
%delete = ();
...
foreach my $bin (sort {$a <=> $b} keys %bam_ins){
    foreach my $pos (sort {$a <=> $b} keys %{$bam_ins{$bin}}){
        my $num = scalar @{${$bam_ins{$bin}}{$pos}};
        my $len = $bam_ins_len{$pos};
        if (($num == 1) and ($len < 200)){
            delete ${$bam_ins{$bin}}{$pos};
        }
        elsif ($len == 0){
            delete ${$bam_ins{$bin}}{$pos};
...
foreach my $bin (sort {$a <=> $b} keys %bam_ins){          # merge similar size of INS with distance 100-500 bp
    foreach my $pos1 (sort {$a <=> $b} keys %{$bam_ins{$bin}}){
        next if (exists $delete{$pos1});
        my $num1 = scalar @{${$bam_ins{$bin}}{$pos1}};
        if ($num1 == 0){
        ...
        if (scalar keys %hit > 0){
            ...
            $hit{$pos1} = $num1;
            my $med_bin = int ($pos1 / $Mbin_size);
            my @len;
            foreach my $pos (keys %hit){
                my $bin2 = int ($pos / $Mbin_size);
                if ($pos != $med_pos){  
                    push @{${$bam_ins{$med_bin}}{$med_pos}}, @{${$bam_ins{$bin2}}{$pos}};
                    delete ${$bam_ins{$bin2}}{$pos};
                    $delete{$pos} = 1;
                }
                foreach (@{${$bam_ins{$bin2}}{$pos}}){
                    my ($id, $strand, $ilen) = split (/=/, $_);
                    push @len, $ilen;
                }
            }
...
for (my $i = 1; $i <= 2; $i++){
    ...
    my $pre_len = 0;
    my $pre_bin = 0;
    foreach my $bin (sort {$a <=> $b} keys %bam_ins){       # merge partially overlapped inss and closely neighbored inss
        foreach my $pos (sort {$a <=> $b} keys %{$bam_ins{$bin}}){
            my $num = scalar @{${$bam_ins{$bin}}{$pos}};
            if ($num == 0){
                delete ${$bam_ins{$bin}}{$pos};
            ...
            my $pre_num = 0;
            $pre_num = scalar @{${$bam_ins{$pre_bin}}{$pre_pos}} if (exists ${$bam_ins{$pre_bin}}{$pre_pos});
            my $len = 0;
            ...
            my $len_rate = int ($len / $pre_len * 100 + 0.5) / 100 if ($pre_len > 0);
            my $distance = $pos - $pre_pos + 1;
            my $match_flag = 0;
            if (($pre_pos > 0) and ($distance <= $bp_diff) and ($distance < $len) and ($distance < $pre_len) and ($len_rate <= 1.7) and ($len_rate >= 0.59)){
                $match_flag = 1;
            }
            elsif (($pre_pos > 0) and ($distance <= $bp_diff) and ($distance < $len) and ($distance < $pre_len) and ($len >= 200) and ($pre_len >= 200) and ($len_rate <= 1.7) and ($len_rate >= 0.59)){
                $match_flag = 1;
            }
            elsif (($pre_pos > 0) and ($distance < $len) and ($distance < $pre_len) and ($len_rate <= 1.1) and ($len_rate >= 0.9)){
                $match_flag = 1;
            }
            if ($match_flag == 1){
                my $len2 = int (($len * $num + $pre_len * $pre_num) / ($num + $pre_num) + 0.5);
                my %ids;
                my $dup_ids = 0;
                foreach (@{${$bam_ins{$bin}}{$pos}}){
                    my ($id) = split (/=/, $_);
                ...
                foreach (@{${$bam_ins{$pre_bin}}{$pre_pos}}){
                    my ($id) = split (/=/, $_);
                    $dup_ids ++ if (exists $ids{$id});
                }
                if (($dup_ids < $num * 0.5) and ($dup_ids < $pre_num * 0.5)){
                    if (($pre_num == 1) and ($num == 1)){
                        my ($pre_id, $pre_strand, $pre_ilen) = split (/=/, ${${$bam_ins{$pre_bin}}{$pre_pos}}[0]);
                        if ($len >= $pre_ilen){
                            push @{${$bam_ins{$bin}}{$pos}}, @{${$bam_ins{$pre_bin}}{$pre_pos}};
                            delete ${$bam_ins{$pre_bin}}{$pre_pos};
                            $len = $len2;
                        ...
                        my $med_pos = $pos[$hnum];
                        my $med_len = $len[$hnum];
                        my @info = (@{${$bam_ins{$pre_bin}}{$pre_pos}}, @{${$bam_ins{$bin}}{$pos}});
                        delete ${$bam_ins{$pre_bin}}{$pre_pos};
                        delete ${$bam_ins{$bin}}{$pos};
                        my $med_bin = int ($med_pos / $Mbin_size);
                        next if ($med_len == 0);
                        push @{${$bam_ins{$med_bin}}{$med_pos}}, @info;
                        $bam_ins_len{$med_pos} = $med_len;
                        $pre_pos = $med_pos;
                        $pre_len = $med_len;
                        $pre_bin = $med_bin;
...
foreach my $bin (sort {$a <=> $b} keys %bam_ins){
    foreach my $pos (sort {$a <=> $b} keys %{$bam_ins{$bin}}){
        ...
        if (scalar keys %keep_id > 0){
            ...
            $med_ilen = $len[$hnum];
            $med_ipos = $pos[$hnum];
            @{${$bam_ins{$bin}}{$pos}} = (@info);
        }
        my @new_info = ();
        foreach (@{${$bam_ins{$bin}}{$pos}}){
            my ($id, $strand, $ilen, $rpos) = split (/=/, $_);
            push @new_info, $_ if ($ilen >= $med_ilen * 0.5) and ($ilen <= $med_ilen * 2);
        }
        my $sum_len = 0;
        foreach (@new_info){
            my ($id, $strand, $ilen) = split (/=/, $_);
        ...
        if (($num < $min_ins_reads) and ($ave_len < 200)){
            delete ${$bam_ins{$bin}}{$pos};
            next;
        }
        if ($num == 1){
            $bam_ins_len{$med_ipos} = $ave_len;
            next;
        }
        ...
        foreach (@{${$bam_ins{$bin}}{$pos}}){
            my ($id, $strand, $ilen) = split (/=/, $_);
            $max_len = $ilen if ($ilen > $max_len);
            $min_len = $ilen if ($ilen < $min_len);
        }
        if (($num == 2) and ($max_len / $min_len > 3)){
            delete ${$bam_ins{$bin}}{$pos};
            next;
        }
        ...
        if ($pos != $med_ipos){
            ...
            my $med_bin = int ($med_ipos / $Mbin_size);
            push @{${$bam_ins{$med_bin}}{$med_ipos}}, @{${$bam_ins{$bin}}{$pos}};
            delete ${$bam_ins{$bin}}{$pos};
        }
    }
}
...
foreach my $bin (sort {$a <=> $b} keys %bam_del){
    foreach my $pos1 (sort {$a <=> $b} keys %{$bam_del{$bin}}){
        my @len1;
        next if (!exists ${$bam_del{$bin}}{$pos1});
        foreach (@{${$bam_del{$bin}}{$pos1}}){
            my ($dlen) = split (/=/, $_);
            push @len1, $dlen;
        }
        my $hn1 = int (@len1 * 0.5);
        ...
        my $end1 = $pos1 + $med_len1;
        my $bin2 = int ($end1 / $Mbin_size);
        my %merge;
        foreach my $pos2 (sort {$a <=> $b} keys %{$bam_del{$bin}}){
            last if ($pos2 > $pos1 + $ins_bp_diff);
            next if ($pos2 <= $pos1);
            next if (!exists ${$bam_del{$bin}}{$pos2});
            my $distance = $pos2 - $pos1 + 1;
            my @len2;
            foreach (@{${$bam_del{$bin}}{$pos2}}){
                my ($dlen) = split (/=/, $_);
                push @len2, $dlen;
            }
            my $hn2 = int (@len2 * 0.5);
        ...
        if (scalar keys %merge > 0){
            ...
            foreach my $dpos (keys %merge){
                if ($dpos != $top_pos){
                    push @{${$bam_del{$bin}}{$top_pos}}, @{${$bam_del{$bin}}{$dpos}};
                    delete ${$bam_del{$bin}}{$dpos};
                }
            }
        }
        %merge = ();
        if ($bin2 > $bin){
            foreach my $pos2 (sort {$a <=> $b} keys %{$bam_del{$bin2}}){
                last if ($pos2 > $pos1 + $ins_bp_diff);
                next if ($pos2 <= $pos1);
                next if (!exists ${$bam_del{$bin2}}{$pos2});
                my $distance = $pos2 - $pos1 + 1;
                my @len2;
                foreach (@{${$bam_del{$bin2}}{$pos2}}){
                    my ($dlen) = split (/=/, $_);
                    push @len2, $dlen;
                }
                my $hn2 = int (@len2 * 0.5);
            ...
            if (scalar keys %merge > 0){
                ...
                foreach my $dpos (keys %merge){
                    if (($dpos != $top_pos) and (exists ${$bam_del{$bin2}}{$dpos})){
                        push @{${$bam_del{$bin2}}{$top_pos}}, @{${$bam_del{$bin2}}{$dpos}};
                        delete ${$bam_del{$bin2}}{$dpos};
                    }
                }
            }
        }
...
foreach my $bin (sort {$a <=> $b} keys %bam_del){       # devide or delete highly deviated size of INSs
    foreach my $pos (sort {$a <=> $b} keys %{$bam_del{$bin}}){ 
        my $max_len = 0;
        my $min_len = 10000000000;
        my @len;
        foreach (@{${$bam_del{$bin}}{$pos}}){
            my ($dlen) = split (/=/, $_);
            if ($dlen > $max_len){
                $max_len = $dlen;
            }
        ...
        if ($med_minlen < $min_indel_size){
            ...
            my $bin2 = int ($med_minpos / $Mbin_size);
            my $bin3 = int ($med_maxpos / $Mbin_size);
            if ($med_minpos == $med_maxpos){
                $med_maxpos ++;
            }
            delete ${$bam_del{$bin}}{$pos};
            push @{${$bam_del{$bin2}}{$med_minpos}}, @max;
            push @{${$bam_del{$bin3}}{$med_maxpos}}, @min;
        }
    }
}
...
for (my $i = 1; $i <= 2; $i++){
    ...
    my $pre_num = 0;
    my $pre_bin = 0;
    foreach my $bin (sort {$a <=> $b} keys %bam_del){   # merge clustered dels for each read
        foreach my $pos (sort {$a <=> $b} keys %{$bam_del{$bin}}){
            my $sumlen = 0;
            foreach (@{${$bam_del{$bin}}{$pos}}){
                my ($dlen) = split (/=/, $_);
            ...
            if (($pre_pos > 0) and ($pos < $pre_end)){
                ...
                if (($overlap >= $pre_len * $min_overlap_rate2) and ($overlap >= $len * $min_overlap_rate2)){
                    push @{${$bam_del{$pre_bin}}{$pre_pos}}, @{${$bam_del{$bin}}{$pos}};
                    delete ${$bam_del{$bin}}{$pos};
                    my $len2 = int ((($pre_len * $pre_num) + ($len * $num)) / ($pre_num + $num) + 0.5);
                    delete $bam_dellen{$pos};
                    $bam_dellen{$pre_pos} = $len2;
                    $pre_num = $pre_num + $num;
                    $pre_end = $pre_pos + $len2 - 1;
                    next;
                }
                else{
                    next if ($i == 1);
    ...
    foreach my $bin (sort {$a <=> $b} keys %bam_del){   # merge clustered dels for each read
        foreach my $pos (sort {$a <=> $b} keys %{$bam_del{$bin}}){ 
            ...
            if (($pre_pos > 0) and ($pos < $pre_end)){
                ...
                my $overlap = $pre_end - $pos + 1;
                $overlap = $len if ($end < $pre_end);
                if (($overlap >= $pre_len * $min_overlap_rate) and ($overlap >= $len * $min_overlap_rate)){
                    push @{${$bam_del{$pre_bin}}{$pre_pos}}, @{${$bam_del{$bin}}{$pos}};
                    delete ${$bam_del{$bin}}{$pos};
                    my $len2 = int ((($pre_len * $pre_num) + ($len * $num)) / ($pre_num + $num) + 0.5);
                    delete $bam_dellen{$pos};
...
for (my $i = 1; $i <= 2; $i++){
    ...
    foreach my $bin (sort {$a <=> $b} keys %bam_del){   # merge clustered dels for each read
        foreach my $pos (sort {$a <=> $b} keys %{$bam_del{$bin}}){
            ...
            my $end = $pos + $len - 1;
            if (($pre_pos > 0) and ($pos <= $pre_end + $ins_bp_diff)){
                my $pre_len = $pre_end - $pre_pos + 1;
                my $len_rate = int ($pre_len / $len * 100 + 0.5) / 100;
                if (($len_rate <= 1.5) and ($len_rate >= 0.67)){
                    push @{${$bam_del{$pre_bin}}{$pre_pos}}, @{${$bam_del{$bin}}{$pos}};
                    delete ${$bam_del{$bin}}{$pos};
                    my $len2 = int ((($pre_len * $pre_num) + ($len * $num)) / ($pre_num + $num) + 0.5);
                    delete $bam_dellen{$pos};
...
foreach my $bin (sort {$a <=> $b} keys %bam_del){       # devide or delete highly deviated size of DELs
    foreach my $pos (sort {$a <=> $b} keys %{$bam_del{$bin}}){
        my $max_len = 0;
        my $min_len = 10000000000;
        my $num = scalar @{${$bam_del{$bin}}{$pos}};
        if ($num < $min_del_reads){
            delete ${$bam_del{$bin}}{$pos};
            next;
        }
        foreach (@{${$bam_del{$bin}}{$pos}}){
        ...
        my $diff = int ($max_len / $min_len * 100 + 0.5) / 100;
        if ((@{${$bam_del{$bin}}{$pos}} == 2) and ($diff > 3)){
            delete ${$bam_del{$bin}}{$pos};
            next;
        }
        next if ($diff <= 1.7);
        ...
        if ($avelen < $min_indel_size){
            ...
            my $bin2 = int ($min_med_pos / $Mbin_size);
            my $bin3 = int ($max_med_pos / $Mbin_size);
            delete ${$bam_del{$bin}}{$pos};
            push @{${$bam_del{$bin2}}{$min_med_pos}}, @min;
            push @{${$bam_del{$bin3}}{$max_med_pos}}, @max;
        }
    }
}
...
foreach my $bin (sort {$a <=> $b} keys %bam_del){
    foreach my $pos (sort {$a <=> $b} keys %{$bam_del{$bin}}){
        my $num = scalar @{${$bam_del{$bin}}{$pos}};
        if ($num == 1){
            my ($dlen) = split (/=/, ${${$bam_del{$bin}}{$pos}}[0]);
            if (($num < $min_del_reads) and ($dlen < 500)){
                delete ${$bam_del{$bin}}{$pos};
                next;
            }
        ...
        if (($num < $min_del_reads) and ($ave_len < 500)){
            delete ${$bam_del{$bin}}{$pos};
            next;
        }
        if ($pos != $med_pos){
            my $bin2 = int ($med_pos / $Mbin_size);
            push @{${$bam_del{$bin2}}{$med_pos}}, @{${$bam_del{$bin}}{$pos}};
            delete ${$bam_del{$bin}}{$pos};
...
if (scalar keys %bam_rep > 0){
    ...
    foreach my $pos (sort {$a <=> $b} keys %bam_rep){
        my $rep_num = scalar @{$bam_rep{$pos}};
        my $ave_dp = 0;
        my $sum_len = 0;
        map{$sum_len += $_} @{$bam_rep{$pos}};
        my $ave_len = int ($sum_len / $rep_num + 0.5);
        my $end = $pos + $ave_len - 1;
        if ($rep_num < $min_ins_reads){
            delete $bam_rep{$pos};
            next;
        }
    }
...
if ($platform ne 'ont'){         
    foreach my $id (keys %read_bp1){    # remove alignments corresponding to unsplit subread (which could lead to miscall of INVs) when realignment had not been conducted in the input bam file
        if (exists $read_bp2{$id}){
            foreach my $end1 (sort {$a <=> $b} keys %{$read_bp1{$id}}){
                foreach my $pos2 (sort {$a <=> $b} keys %{$read_bp2{$id}}){
                    if (abs ($end1 - $pos2) <= $bp_diff2){
                        my ($maplen1, $sublen1, $cigar1, $strand1) = split (/=/, ${$read_bp1{$id}}{$end1});
                        my ($maplen2, $sublen2, $cigar2, $strand2) = split (/=/, ${$read_bp2{$id}}{$pos2});
                        if ($strand1 ne $strand2){
                            ${$unsplit_subread1{$id}}{$end1} = $strand1;
                            ${$unsplit_subread2{$id}}{$pos2} = $strand2;
                        }
                    }
                }
            }
    ...
    print STDERR "Reads with unsplit subreads: ", scalar keys %unsplit_subread1, "\n";
}
...
#                next if ($strand1 ne $strand2);
                ...
                my $bp1_bin = int ($bp1 / $Mbin_size);
                my $bp2_bin = int ($bp2 / $Mbin_size);
                my $min_bin = $bp1_bin;
                $min_bin = $bp1_bin if ($bp2_bin < $bp1_bin);
                $min_bin -- if ($min_bin > 0);
                my $max_bin = $bp1_bin;
                $max_bin = $bp2_bin if ($bp2_bin > $bp1_bin);
                $max_bin ++;
                my $ins_match = 0;
                foreach my $bin (sort {$a <=> $b} keys %bam_ins){
                    next if ($bin < $min_bin);
                    last if ($bin > $max_bin);
                    foreach my $ipos (sort {$a <=> $b} keys %{$bam_ins{$bin}}){
                        last if ($ipos > $bp1 + $bp_diff);
                        next if ($ipos < $bp2 - $bp_diff);
                        my $ilen = $bam_ins_len{$ipos};
                        next if ($ilen < $distance);
                        my $rnum = scalar @{${$bam_ins{$bin}}{$ipos}};
...
foreach my $bp1 (sort {$a <=> $b} keys %clust_2bp_hcov2){   # detect >= 4 Kb distance double-clipped reads as DUP
    my $bp1_p = int ($bp1 / 10 + 0.5) * 10;
    next if (exists $dup_clust{$bp1_p});
    my @bp2;
...
foreach my $pos1 (sort {$a <=> $b} keys %dup_clust){        # filter overlapped DUPs
    ...
    if (($pre_pos > 0) and ($pos1 < $pre_end)){
        ...
        if (($pos2 < $pre_end) or (($overlap >= $len * $min_overlap_rate) and ($overlap >= $pre_len * $min_overlap_rate))){
            my $num = scalar @{${$dup_clust{$pos1}}{$pos2}};
            next if (!exists ${$dup_clust{$pre_pos}}{$pre_end});
...
foreach my $pos1 (sort {$a <=> $b} keys %inv_clust){        # filter overlapped INVs
    ...
    if (($pre_pos > 0) and ($distance <= $len) and ($distance <= $pre_len)){
        ...
        my $num = scalar @{${$inv_clust{$pos1}}{$pos2}};
        next if (!exists ${$inv_clust{$pre_pos}}{$pre_pos2});
...
foreach my $bp1 (sort {$a <=> $b} keys %inv_clust){
    if (scalar keys %{$inv_clust{$bp1}} == 0){
        delete $inv_clust{$bp1};
        next;
    }
        ...
        if (exists $read_bp2{$id}){     # assign DELs/REPs with two clustered breakpoints (Minialign2 maps also clipped read regions with the same read id)
            ...
            foreach my $bp2 (sort {$a<=> $b} keys %{$read_bp2{$id}}){
                ...
                if ($distance >= $bp_diff2){   # check whether the aligned length at the bp1 and the clipped length at the bp2 of the same read alignment are similar
                    ...
                    foreach my $dbp1 (sort {$a <=> $b} keys %bp_id){
                        last if ($dbp1 > $bp1_p + $bp_diff2);
                        next if ($dbp1 < $bp1_p - $bp_diff2);
                        foreach my $dbp2 (sort {$a <=> $b} keys %{$bp_id{$dbp1}}){
                            last if ($dbp2 > $bp2_p + $bp_diff2);
                            next if ($dbp2 < $bp2_p - $bp_diff2);
                            $overlap_flag = 1;
                        }
                    }
                    next if ($overlap_flag == 1);
                    ${$del_bp{$bp1_p}}{$bp2_p} ++;
                    ${$bp_id{$bp1_p}}{$bp2_p} = 1;
                    ...
                    if (($ins_len >= 200) and ($ins_len < 5000)){
                        ...
                        foreach my $ibp1 (sort {$a <=> $b} keys %ibp_id){
                            last if ($ibp1 > $bp_m + $bp_diff);
                            next if ($ibp1 < $bp_m - $bp_diff);
                            $overlap_flag = 1;
                        }
                        next if ($overlap_flag == 1);
...
foreach my $bp1 (sort {$a <=> $b} keys %bam_del_bp){        # remove BP-DELs overlapping assigned DELs
    ...
    my $bp2 = $bp1 - 500;
    my $bp3 = $bp1 + 20;
    my $bin2 = int ($bp2 / $Mbin_size);
    my $bin3 = int ($bp3 / $Mbin_size);
    foreach my $dpos (sort {$a <=> $b} keys %{$bam_del{$bin2}}){
        last if ($dpos > $bp1 + 20);
    ...
    if ($bin3 > $bin2){
        foreach my $dpos (sort {$a <=> $b} keys %{$bam_del{$bin3}}){
            last if ($dpos > $bp1 + 20);
            next if ($dpos < $bp1 - 500);
            my $dlen = $bam_del_len{$dpos};
            if (($blen / $dlen >= 0.8) and ($blen / $dlen <= 1.2)){
...
foreach my $bp1 (sort {$a <=> $b} keys %rep_bp){
    ...
    if ($select_bp2 != 0){
        ...
        $bam_rep_bp{$bp1_med} = "$repsize=$read_num=$read_num";
        $DEL{$bp1_med} = 1;
        $used{$bp1_med} = 1;
    }
}
...
foreach my $bp1 (sort {$a <=> $b} keys %ins_bp_inread){
    ...
    my $bp1_bin = int ($bp1 / $Mbin_size);
    foreach my $bin (sort {$a <=> $b} keys %bam_ins){
        next if ($bin < $bp1_bin - 1);
        last if ($bin > $bp1_bin + 1);
        foreach my $bp2 (sort {$a <=> $b} keys %{$bam_ins{$bin}}){
            last if ($bp2 > $bp1 + 100);
            next if ($bp2 < $bp1 - 100);
            my $dist = abs ($bp1 - $bp2);
            $idist{$dist} = $bp2;
...
foreach my $bp1 (keys %bam_dup){
    my $del_hit = 0;
    my $bin1 = int ($bp1 / $Mbin_size);
    foreach my $dpos (sort {$a <=> $b} keys %{$bam_del{$bin1}}){
...
foreach my $bin (sort {$a <=> $b} keys %bam_del){
    foreach my $pos (sort {$a <=> $b} keys %{$bam_del{$bin}}){
        my $sum_len = 0;
        my $max_len = 0;
        my $read_num = scalar @{${$bam_del{$bin}}{$pos}};
        foreach my $dlen_pos (@{${$bam_del{$bin}}{$pos}}){
            my ($dlen, $dpos) = split (/=/, $dlen_pos);
            $sum_len += $dlen;
...
foreach my $bp1 (sort {$a <=> $b} keys %bam_bp1){
    ...
    if (($pre_bp1 > 0) and ($bp1 - $pre_bp1 <= 30)){
...
foreach my $bp2 (sort {$a <=> $b} keys %bam_bp2){
    ...
    if (($pre_bp2 > 0) and ($bp2 - $pre_bp2 <= 30)){
...
foreach my $pos (sort {$a <=> $b} keys %ins_bp12){      # assign INS to INS-BP with BPLEN or INS-BP to DUP
    ...
    if ($bplen > 100){
        ...
        my $Mbin2 = int ($pos2 / $Mbin_size);
        foreach my $ipos (sort {$a <=> $b} keys %{$bam_ins{$Mbin1}}){
            ...
            if (($ilen >= 200) and ($ilen > $bplen * 0.5)){
                push @{$ins_info{$ipos}}, @{${$bam_ins{$Mbin1}}{$ipos}};
                if ($ilen > $max_len){
                    $max_len = $ilen;
                }
            }
        ...
        if ($Mbin2 > $Mbin1){
            foreach my $ipos (sort {$a <=> $b} keys %{$bam_ins{$Mbin2}}){
                last if ($ipos > $pos2 + 20);
                next if ($ipos < $pos - 20);
                my $ilen = $bam_ins_len{$ipos} if (exists $bam_ins_len{$ipos});
                if (!exists $bam_ins_len{$ipos}){
                ...
                if (($ilen >= 200) and ($ilen > $bplen * 0.5)){
                    push @{$ins_info{$ipos}}, @{${$bam_ins{$Mbin2}}{$ipos}};
                    if ($ilen > $max_len){
                        $max_len = $ilen;
                    }
                }
        ...
        if ($max_len > 0){
            ...
            foreach my $ipos (sort {$a <=> $b} keys %ins_info){
                foreach (@{$ins_info{$ipos}}){
                    ...
                    if (!exists $ins_ids{$id}){
                        ...
                        my ($id2, $strand2, $ilen2, $rpos2, $ipos2, $mapq2) = split (/=/, $ins_ids{$id});
                        $ilen2 += $ilen;
                        my $new_inf = "$id2=$strand2=$ilen2=$rpos2=$ipos2=$mapq2";
                        $ins_ids{$id} = $new_inf;
                        $ins_ids_dup{$id} = 1;
                    }
                    my $bp_flag = 0;
            ...
            if (@select_info == 0){
                ...
                foreach my $ipos (keys %ins_info){
                    my $Mbin = int ($ipos / $Mbin_size);
                    delete ${$bam_ins{$Mbin}}{$ipos};
                }
            }
            else{
                delete ${$bam_ins{$Mbin1}}{$pos} if (exists ${$bam_ins{$Mbin1}}{$pos});
                    ...
                    if ($med_minlen >= $bplen * 0.5){
                        ...
                        foreach my $ipos (keys %ins_info){
                            ...
                            if (($ilen / $med_minlen >= 0.8) and ($ilen / $med_minlen <= 1.25)){
                                my $Mbin = int ($ipos / $Mbin_size);
                                push @{${$bam_ins{$Mbin}}{$ipos}}, @{$ins_info{$ipos}} if ($ipos != $pos);
                            }
                        }
                    }
                }
...
my %bam_ins_BP;
foreach my $bin (sort {$a <=> $b} keys %bam_ins){           # assign >200 bp INS to the separated BPs of 5'- and 3'-ends
    foreach my $pos (sort {$a <=> $b} keys %{$bam_ins{$bin}}){
        my $read_num = scalar @{${$bam_ins{$bin}}{$pos}};
        next if ($read_num == 0);
        next if (exists $bam_ins_bp2{$pos});
        my $ilen = 0;
        if (exists $bam_ins_len{$pos}){
            $ilen = $bam_ins_len{$pos};
        }
        else{
            my @len;
            foreach (@{${$bam_ins{$bin}}{$pos}}){
        ...
        if (($top_bp1 == 0) and (abs ($top_bp2 - $pos) <= 50) and ($bp2_read >= 3)){
            $top_bp1 = $pos;
        }
        elsif (($top_bp2 == 0) and (abs ($top_bp1 - $pos) <= 50) and ($bp1_read >= 3)){
            $top_bp1 = $pos;
        }
        next if ($top_bp1 == 0) or ($top_bp2 == 0);
        my $bplen = $top_bp2 - $top_bp1 + 1;
        next if ($bplen > $ilen * 2);
        my $bp1_3T = 0;
        my $bp2_5T = 0;
        if ($bplen >= 100){
            foreach my $bp1 (sort {$a <=> $b} keys %bam_bp1){
                last if ($bp1 > $top_bp1 + 50);
                next if ($bp1 < $top_bp1 - 50);
                $bp1_3T ++;
            }
            foreach my $bp2 (sort {$a <=> $b} keys %bam_bp2){
                last if ($bp2 > $top_bp2 + 50);
                next if ($bp2 < $top_bp2 - 50);
                $bp2_5T ++;
            }
        ...
        if (($bp1_read >= 2) and ($bp2_read >= 2) and ($bplen >= 100)){
...
foreach my $bp_range (keys %bam_ins_BP){
    ...
    foreach (@{$bam_ins_BP{$bp_range}}){
        ...
        my $Mbin = int ($ipos / $Mbin_size);
        next if (!exists ${$bam_ins{$Mbin}}{$ipos});
        foreach (@{${$bam_ins{$Mbin}}{$ipos}}){
            my ($id, $strand, $ilen, $rpos) = split (/=/, $_);
            $read_ilen{$id} += $ilen;
    ...
    if ($max_len == 0){
        ...
        foreach my $ipos (keys %ipos){
            my $Mbin = int ($ipos / $Mbin_size);
            next if (!exists ${$bam_ins{$Mbin}}{$ipos});
            push @info, @{${$bam_ins{$Mbin}}{$ipos}};
        }
        delete ${$bam_ins{$Mbin1}}{$bp1} if (exists ${$bam_ins{$Mbin1}}{$bp1});
        push @{${$bam_ins{$Mbin1}}{$bp1}}, @info;
        foreach (@{$bam_ins_BP{$bp_range}}){
            my ($ipos, $ilen) = split (/=/, $_);
            next if ($ipos == $bp1);
...
foreach my $pos1 (sort {$a <=> $b} keys %bam_dup){              # assign INS within DUP to DUP
    foreach my $pos2 (sort {$a <=> $b} keys %{$bam_dup{$pos1}}){
        ...
        %dup_ins = ();
        if ($gt eq 'HM'){
            my $Mbin1 = int ($pos1 / $Mbin_size);
            my $Mbin2 = int ($pos2 / $Mbin_size);
            foreach my $ipos (sort {$a <=> $b} keys %{$bam_ins{$Mbin1}}){
                last if ($ipos > $pos2 + 50);
                next if ($ipos < $pos1 - 50);
                if (exists ${$bam_ins{$Mbin1}}{$ipos}){
                    foreach (@{${$bam_ins{$Mbin1}}{$ipos}}){
                        my ($id, $strand, $ilen) = split (/=/, $_);
                        $read_len{$id} += $ilen;
            ...
            if ($Mbin2 > $Mbin1){
                foreach my $ipos (sort {$a <=> $b} keys %{$bam_ins{$Mbin2}}){
                    last if ($ipos > $pos2 + 50);
                    next if ($ipos < $pos1 - 50);
                    if (exists ${$bam_ins{$Mbin2}}{$ipos}){
                        foreach (@{${$bam_ins{$Mbin2}}{$ipos}}){
                            my ($id, $strand, $ilen) = split (/=/, $_);
                            $read_len{$id} += $ilen;
                            $read_pos{$id} = $ipos if (!exists $read_pos{$id});
                            $ins_pos{$ipos} = $id;
...
foreach my $bin (sort {$a <=> $b} keys %bam_ins){
    foreach my $pos (sort {$a <=> $b} keys %{$bam_ins{$bin}}){
        ...
        next if ($read_num == 0);
        foreach (@{${$bam_ins{$bin}}{$pos}}){
            my ($id, $strand, $len, $rpos, $ipos, $mapq) = split (/=/, $_);
            $read_info .= "$_|";
            $clust_id{$id} = 1;
            if ($strand ne 'BP'){
                push @len, $len;
                push @mapq, $mapq if (defined $mapq);
            }
        }
        map{$sum_len += $_} @len;
        my $BP = 0;
        ...
        $BP = 0 if ($bp1 < 1) or ($bp2 < 1);
        $BP = 0 if ($bp1 <= 1) and ($bp2 <= 1);
        $BP = 0 if ($bp1 <= 2) and ($bp2 <= 2) and ($platform ne 'ont');
        my $bplen = 0;
        $bplen = $pos2 - $pos + 1 if ($pos2 > 0);
        my $read_num2 = scalar keys %clust_id;
        next if ($read_num2 == 1) and ($BP <= 4) and ($platform ne 'ont');
        $read_num2 += $BP;
        my $ave_len = int ($sum_len / @len + 0.5);
        next if ($read_num2 == 1);
        my $ave_mapq = 30;
        if (@mapq > 0){
            map{$sum_mapq += $_} @mapq;
            $ave_mapq = int ($sum_mapq / @mapq + 0.5);
        }
        $read_info =~ s/\|$//;
        my $hnum = int (@len * 0.5);
        @len = sort {$a <=> $b} @len;
        my $med_len = $len[$hnum];
        if (($ave_len >= 200) and ($BP == 0)){
            my $bp5 = 0;
            my $bp3 = 0;
            my $max_len = 1000;
            $max_len = $ave_len if ($ave_len < $max_len);
            for (my $i = $pos - 100; $i <= $pos + $max_len; $i++){
                if ((exists $bam_bp1{$i}) and ($i >= $pos - 30)){
                    my $num = scalar @{$bam_bp1{$i}};
                    $bp3 = $num if ($num > $bp3);
                }
                if ((exists $bam_bp2{$i}) and ($i <= $pos + 30)){
                    my $num = scalar @{$bam_bp2{$i}};
                    $bp5 += $num;
                }
            }
            if (($bp5 > 1) and ($bp3 > 1)){
                $BP = int (($bp5 + $bp3) * 0.5 + 0.5);
            }
            $read_num2 += $BP;
        }
        my $dp = 0;
        ...
        next if ($ins_rate < $min_ins_VRR);
        next if ($ins_rate < $min_VRR * 1.5) and ($med_len <= 10);
        next if ($ins_rate < $min_VRR * 2) and ($med_len <= 5);
        print OUT "$chr\t$pos\tINS\t$med_len\t$read_info\t$read_num2,$dp,$ins_rate,$BP,$bplen,$dp_rate,$ave_mapq\n";
        $total_ins_it ++;
    }
}
foreach my $pos (sort {$a <=> $b} keys %bam_ins_bp){
...
foreach my $pos (sort {$a <=> $b} keys %ins_bp12){      # INSs supported by reads with either 5'-clipped or 3'-clipped alignment at the BPs
    ...
    foreach my $pos2 (sort {$a <=> $b} keys %{$ins_bp12{$pos}}){
        my $clip_tag = '3T' if ($pos2 == 1);
        $clip_tag = '5T' if ($pos2 > 1);
        foreach my $info (@{${$ins_bp12{$pos}}{$pos2}}){
            my @info = split (/=/, $info);
            next if (@info < 6);
            my ($readid, $strand, $cliplen, $tag, $maplen, $bp) = split (/=/, $info);
            $S5_clip ++ if ($clip_tag eq '5T') and ($tag eq 'S');
            $S3_clip ++ if ($clip_tag eq '3T') and ($tag eq 'S');
            $read_info .= "$readid=$strand=$cliplen=$clip_tag=$tag=$bp|";
        }
    }
    next if ($S5_clip <= 1) or ($S3_clip <= 1) and ($platform eq 'ont');
    next if ($S5_clip <= 2) or ($S3_clip <= 2) and ($platform ne 'ont');
    ...
    for (my $i = $pos - 50; $i <= $pos + 50; $i++){
        ...
        if (exists ${$bam_del{$Mbin}}{$i}){
            my @len;
            my $sumlen = 0;
            foreach (@{${$bam_del{$Mbin}}{$i}}){
                my ($dlen) = split (/=/, $_);
                push @len, $dlen;
            }
            if (@len > 0){
...
foreach my $bin (sort {$a <=> $b} keys %bam_del){
    foreach my $pos (sort {$a <=> $b} keys %{$bam_del{$bin}}){
        ...
        my @mapq;
        foreach (@{${$bam_del{$bin}}{$pos}}){
            my ($dlen, $dpos, $id, $mapq) = split (/=/, $_);
            push @len, $dlen;
            push @mapq, $mapq;
            $max_len = $dlen if ($max_len < $dlen);
            $read_count ++;
            $del_reads .= "$id=$dlen=$dpos,";
        }
        $del_reads =~ s/,$//;
        map{$sum_len += $_} @len;
        my $hnum = int (@len * 0.5);
        ...
        my $ave_len = int ($sum_len / $read_num + 0.5);
        map{$sum_mapq += $_} @mapq;
        my $ave_mapq = int ($sum_mapq / @mapq + 0.5);
        my $end = $pos + $max_len - 1;
        my $bp_len = 0;
        my $bp_num_str = '';
        my $bp_num = 0;
        ...
        print OUT "$chr\t$pos\tDEL\t$ave_len\t$read_num\t$del_reads\t$dp_rate,$flank_dp,$del_rate,$BP\t$ave_mapq\n";
        $total_del_it ++;
    }
}
...
foreach my $pos (sort {$a <=> $b} keys %bam_indel_str){
    ...
    if (($type1 ne 'NA') and ($type2 ne 'NA')){
        if (($type1 eq 'DEL') and ($type2 eq 'INS')){
            my $read_ipos = '';
            if (exists ${$bam_ins_str_read{$pos}}{2}){
                my $count = 0;
                foreach (@{${$bam_ins_str_read{$pos}}{2}}){
                    $count ++;
                    $read_ipos .= "$_|";
                    last if ($count == 5);
                }
            ...
            my $read_ipos = '';
            if (exists ${$bam_ins_str_read{$pos}}{1}){
                my $count = 0;
                foreach (@{${$bam_ins_str_read{$pos}}{1}}){
                    $count ++;
                    $read_ipos .= "$_|";
                    last if ($count == 5);
                }
            ...
            $read_ipos1 = 'NA' if ($read_ipos1 eq '');
            $read_ipos2 = 'NA' if ($read_ipos2 eq '');
...
foreach my $pos (sort {$a <=> $b} keys %bam_rep_bp){
    my ($rlen, $bp1_num, $bp2_num) = split (/=/, $bam_rep_bp{$pos});
    ...
    next if ($read_num < $min_ins_reads);
    next if ($rep_rate < $min_VRR);
    print OUT "$chr\t$pos\tREP-BP\t$rlen\t$bp1_num,$bp2_num\tNA\t$dp,$rep_rate\n";
}
...
print STDERR "Removed low align rate (< $min_align_rate) reads  $chr3: $delete_alignrate\n";
