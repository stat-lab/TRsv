...
Alternatively, a singularity container file (TRsv-v1.1.2.sif) is available [TRsv-v1.1.2.sif](https://drive.google.com/uc?export=download&id=1IS8PGQ9NsdJaHMBUwRVf5iL2NUb1Ng5P). Also, the singularity container can be created through the Docker Hub: "singularity build TRsv.sif docker://shunichikosugi/trsv:latest". Run using the singularity TRsv-v1.1.2.sif file can be done using a run_TRsv_singularity.pl script in the scripts directory or simply executed with: "singularity exec TRsv.sif TRsv call (or other commands)".
